...
@Component({
  selector: "app-case-planning-metrics",
  templateUrl: "./case-planning-metrics.component.html",
  styleUrls: ["./case-planning-metrics.component.scss"]
})
export class CasePlanningMetricsComponent implements OnInit {
  // inputs
  @Input() metrics;
  @Input() weekOf;
      ...
      if (metricsId === "supply") {
        ...
        for (var j in dataLevelOne) {
          ...
          dataLevelOne[j]["levels"]?.forEach((level) => {
            const supplyAvailableCountForLevel = this.metrics[0].data[j].available[0].levels?.find(
              (x) => x.name === level.name
            ).sum;
            const supplyProspectiveCountForLevel = this.metrics[0].data[j].prospective[0].levels?.find(
              (x) => x.name === level.name
            ).sum;
            const supplyTotalCountForLevel =
              Number(parseFloat(supplyAvailableCountForLevel)) + Number(parseFloat(supplyProspectiveCountForLevel));
            const demandTotalCountForLevel = Number(
              parseFloat(
                this.metrics[1].data[j].levels?.find((x) => x.name === level.name)
                  ? this.metrics[1].data[j].levels?.find((x) => x.name === level.name)["sum"]
                  : 0
              )
            );
      ...
      if (metricsId === "staffableTeams") {
        ...
        this.metrics[i]["sum"] = dataLevelOne?.reduce(
          (previousValue, currentValue) =>
            (Number(parseFloat(previousValue)) + Number(parseFloat(currentValue.sum))).toFixed(2),
          initialValue
        );
      }
    }
  ...
  getSumForSupplyLevels(data, initialValue) {
    ...
    levels?.forEach((item) => {
      levelSum = item.levelGrades.reduce(
        (previousValue, currentValue) =>
          (Number(parseFloat(previousValue)) + Number(parseFloat(currentValue.supply))).toFixed(2),
        initialValue
      );
