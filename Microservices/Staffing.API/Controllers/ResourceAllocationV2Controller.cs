...
namespace Staffing.API.Controllers
{
    ...
    public class ResourceAllocationV2Controller : ControllerBase
    {

        private readonly IResourceAllocationService _resourceAllocationService;

        public ResourceAllocationV2Controller(IResourceAllocationService resourceAllocationService)
        {
        ...
        public async Task<IActionResult> GetResourceAllocationsBySelectedValues(dynamic payload)
        {
            ...
            var response = await _resourceAllocationService.GetResourceAllocationsBySelectedValuesV2(oldCaseCodes,
            employeeCodes, lastUpdated, startDate, endDate, caseRoleCodes, clientId, action);
