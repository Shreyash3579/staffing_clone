...
export class GanttCaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subscribeToChangesInAllocationData() {
    ...
    this._ganttService.resourceAndPlaceholderAllocations.pipe(takeUntil(this.destroy$)).subscribe(allocations => {
      ...
      this.showCommitmentsForResources();
  ...
  showCommitmentsForResources() {
    if (this.showCommitments && this.userAllocation !== null) {
      ...
      const commitmentTypesToShowInGantt = (new Array(ConstantsMaster.CommitmentType.Loa.toLowerCase(),
        ...
        ConstantsMaster.CommitmentType.Commitment.toLocaleLowerCase(),
  ...
  private bindAngularEventsToGantt() {
    ...
    this.ganttCase$.getCommitmentDataFromGanttTask = this.getCommitmentDataFromGanttTask;
  ...
  updateResource(item) {
    ...
    if(item.commitmentType != null){
      this.updateResourcesCommitment(item, startDate, endDate);
    }

    else {
      //check here if it is a commitment, then call that method
  ...
  updateResourcesCommitment(item, startDate, endDate) {
    const commitmentType = this.commitmentTypes.find(
      ct => ct.commitmentTypeCode === item.commitmentType
    );
    ...
    const updatedUserCommitment: Commitment = {
            ...
            commitmentTypeReasonCode: item.commitmentTypeReasonCode || null,
            lastUpdatedBy: null,
  ...
  getCommitmentDataFromGanttTask(data)
  {
    return data.map(r => {
      const commitmentType = this.commitmentTypes?.find(
        ct => ct.commitmentTypeCode === r.commitmentType
      );
      const allocation = parseInt(r.allocation, 10);
      ...
      return {
        ...
        commitmentType: {
          commitmentTypeCode: commitmentType?.commitmentTypeCode ?? r.commitmentType,
          commitmentTypeName: commitmentType?.commitmentTypeName,
          precedence: commitmentType?.precedence
        },
        notes: r.notes,
  ...
  private getAllocationDataFromGanttTask(data) {
    return data.map(r => {
      return {
        ...
        commitmentTypeCode: r.commitmentType,
  ...
  updateDeleteAllocations(selectedAllocationsAndCommitments, selectedOption) {
    const selectedCommitments = selectedAllocationsAndCommitments.filter(x=> x.commitmentType && x.commitmentType !== 'C');
    const selectedCommitmentsFromGanttTask = this.getCommitmentDataFromGanttTask(selectedCommitments);
    
    const selectedAllocations = selectedAllocationsAndCommitments.filter(x=> !x.commitmentType || x.commitmentType === 'C');
    const selectedAllocationsFromGanttTask = this.getAllocationDataFromGanttTask(selectedAllocations);
    
    if (selectedAllocationsAndCommitments.some(x => !x.isPlaceholderAllocation)) {
      if (selectedOption === 'Delete') {
        this.deleteSelectedProjectsConfirmationDialog.emit({resourceAllocations : selectedAllocationsFromGanttTask, resourceCommitments: selectedCommitmentsFromGanttTask});
      } else {
        this.openUpdateAllocationsDatesDialog.emit({ title: selectedOption, resourceAllocations: selectedAllocationsFromGanttTask, resourceCommitments: selectedCommitmentsFromGanttTask });
      }
    } else {
      if (selectedOption === 'Delete') {
  ...
  addContextMenuItem(title, selectedAllocationsAndCommitments, value) {
    ...
    if (selectedAllocations.some(x => x.isPlaceholderAllocation) && selectedAllocations.some(x => !x.isPlaceholderAllocation)) {
    ...
    else if(value === 'Split' && selectedAllocations.length == 0){
        startDateLi.style.color = '#bababa';
        startDateLi.title = 'Cannot split commitment';
    } 
      ...
      if (selectedAllocations.length > 1 || selectedCommitments.length > 0 ||
      ...
      startDateLi.addEventListener('click', (e) => {
        ...
        //update allocations and commitments
        this.updateDeleteAllocations(selectedAllocationsAndCommitments, clickedOption.getAttribute('value'));
        document.getElementById('ganttCaseContextMenuId').innerHTML = '';
      });
    }
